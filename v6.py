...
LANDMARKS = {
    ...
    "Inion":[  0.3300,  103.3470,  -0.6190],
    "dlPFC":[ 68.5660,  -52.1200,   34.4291],
    "leftHelixRoot":  [ 87.0179,  -2.86688, -25.0818],
    "rightHelixRoot": [-86.2151,  -3.22943, -22.9474],
}
...
def resolve_mesh_path() -> str:
    ...
    print("SCALED_MODEL NOT FOUNDDDD!!!!!")
...
def resolve_landmarks_with_custom(default_landmarks: dict) -> dict:
    ...
    mapping = {'Fpz': 'FPZ', 'Inion': 'INION', 'dlPFC': 'DLPFC'}
    ...
    if missing_any:
        print("CUSTOM VARIABLES NOT FOUND!!!!!!!!!!!!!!!!!!!!!!!!!")
...
#      A: through (Cz, Inion, Fpz)
#      H: through (Inion↔Fpz), perpendicular to A
#      C: through (dlPFC↔Cz), perpendicular to H
#      inter_BC: first scalp hit along line H∩C
# ======================================================================
def compute_planes_and_interbc(mesh, pos, pq):
    nA = np.cross(pos['Fpz'] - pos['Cz'], pos['Inion'] - pos['Cz']); nA /= (np.linalg.norm(nA) + EPS)
    line_FI = pos['Fpz'] - pos['Inion']; line_FI /= (np.linalg.norm(line_FI) + EPS)
...
#     Rays: mid→Inion and mid→inter_BC (sector not visualized; orbs only)
# ======================================================================
...
# ======================================================================
def main():
    with tqdm(total=10, desc="Pipeline", ncols=92) as pb:
        # [3] Mesh
        pb.set_description("[3] Resolve & prep mesh"); pb.update()
        mesh_path = resolve_mesh_path()
        mesh = load_and_prep_mesh(mesh_path)

        # Resolve landmarks with optional caller overrides (FPZ/INION/DLPFC)
        ...
        # [8] Lengths (before render)
        pb.set_description("[8] Print lengths"); pb.update()
        print_lengths(lengthV, lengthH)
